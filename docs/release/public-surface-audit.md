...
| GitHub Actions/Pages policy | Pass | GitHub Pages is built and deployed by Actions from `docs_site/` through `python -m mkdocs build --strict`, `actions/upload-pages-artifact`, and `actions/deploy-pages`. No `gh-pages` branch deployment or `mkdocs gh-deploy` workflow was found. |
